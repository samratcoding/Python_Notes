...
- If not start with decimal then it is local, not global
```
If IPv6 Global missing
```
...
network:
  ...
  ethernets:
   eth0:
     addresses:
      - 103.193.72.36/24   # Your public IPv4
      - 2001:db8::2/64     # IPv6 from Tunnel Broker
     routes:
      - to: default
        via: 103.193.72.1  # Default IPv4 gateway
      - to: default
        via: 2001:db8::1   # Default IPv6 gateway from tunnel broker
     nameservers:
      addresses:
        - 8.8.8.8           # IPv4 DNS
        - 2001:4860:4860::8888 # IPv6 DNS (Google)
```
```
sudo netplan apply
