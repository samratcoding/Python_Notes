...
# Define the number of concurrent visits allowed
...
async def main(links):
    async with async_playwright() as playwright:
        ...
        context = await browser.new_context(user_agent="Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/92.0.4515.107 Safari/537.36")

        semaphore = asyncio.Semaphore(MAX_CONCURRENT_VISITS)  # Create a semaphore to limit concurrency
...
# https://playwright.azureedge.net/builds/chromium/1124/chromium-win64.zip
chrome_path = Path(__file__).parent.joinpath('chrome-win/chrome.exe')
...
storage_state_file = Path(__file__).parent.joinpath('storage_state.json')
...
```
## 08. Creating New Page & Visiting individual page
```py
from playwright.sync_api import sync_playwright
with sync_playwright() as p:
    browser = p.chromium.launch(headless=False, args=['--disable-blink-features=AutomationControlled'],)
...
```
## 19. DOM Content Update & JavaScript
```py
...
```
## 20. How to intertact JS click from browser devs
```
...
```
## 22. Hover and hold or click  and hold
```py
...
```
## 29. Load and Automate extension
```py
